...
pnpm check clean.
 ...
 c5e10c8 (Live chat transcript stacks in true arrival order)
...
2026-09-21 - Pool-overload fallback: inference survives z.ai free-pool congestion

Z.ai's free glm-4.7-flash pool congests for hours at peak (error 1305, HTTP 429), which surfaced to users as a failed reply even though glm-4.5-flash - the other free flash model - serves fine on a separate pool (verified: 3/3 plain calls + clean tool call while 4.7 was down). The gateway now wraps every chat attempt (chatWithMistralGateway AND completeWithMistralGateway - the latter serves agent summaries, automations, and the router) in attemptWithPoolFallback: a rate_limit failure on the primary model retries once on the configured pool fallback (ZAI_FALLBACK_MODEL, defaulting to glm-4.5-flash in Z.ai mode since /models omits free models), arms a 5-minute degradation window during which new requests skip straight to the fallback, and clears the window early if the fallback pool is also overloaded so the primary is retried. The fallback attempt reuses the run's allowance claim, so it is not double-charged. Three new tests pin the retry-on-overload, degraded-routing, and window-clearing paths (28 gateway tests, 493 total).

2026-09-21 - Failure replies now carry the actual backend error

The canned inference-failure replies (configuration, allowance_reached, rate_limit, client_error, invalid_response) hid the upstream truth behind default text: during the first live Z.ai run the user saw "Mistral AI's rate limit was hit" while Z.ai had actually answered 1113 "Insufficient balance or no resource package. Please recharge" (paid model via degradation). Every canned failure reply now appends "Actual backend error: <message | cause chain>" capped at 500 chars, and the wording is provider-neutral ("The inference provider's rate limit..." etc.) since the gateway is provider-agnostic (PR #121). Tests pin the actual-error text on the rate-limit, client-error, and configuration paths (490 total, 98 in the agent suite).

2026-09-21 - Z.ai main-agent rollout: the free flash default must not degrade to a paid model

- First live Z.ai run (Telegram, 04:30) failed with Z.ai error 1113 "Insufficient balance or no resource package" - which z.ai sends as HTTP 429, so the gateway's rate-limit retry classification made it look like a lockout. Root cause: z.ai's /models endpoint lists only paid models (glm-4.5 ... glm-5.3-flashx) and omits the free flash models, so the gateway's discovery-based degradation discarded the configured glm-4.7-flash default and fell to the first listed model, glm-4.5 - paid, instant balance wall. Direct key tests confirmed: glm-4.7-flash chat and function calling both work on the free tier (tool_calls returned with reasoning_content), glm-5.3-flash and glm-4.5 return 1113 on this zero-balance account. Fix: in Z.ai mode a configured ZAI_DEFAULT_MODEL is authoritative without the served-check; the Mistral side keeps the served-check because Mistral's /models list is authoritative. New test pins the unlisted-free-model scenario (25 gateway tests, 487 total).
- Free-pool caveat learned the hard way: z.ai's free glm-4.7-flash pool congests at peak (error 1305, also HTTP 429) - plain chats and tool calls intermittently fail for minutes, then recover. The gateway's existing one-patient-retry handles single blips; sustained peak-hour congestion will surface as patient retries that still fail. That is the free-tier tradeoff (as with Mistral free), not a bug: GLM-4.7-Flash is confirmed completely free per z.ai docs.

2026-09-21 - gateway env vars: Z.ai gets its own names, Mistral becomes the fallback

- PR #121 made the gateway's default chat model env-overridable, but the deployment-facing env var names still carried the MISTRAL_ prefix even when the transport points at Z.ai - confusing for whoever manages the Vercel project. The gateway now has two full, mode-selected configurations: setting ZAI_API_KEY (or NOVA_ZAI_GATEWAY_TOKEN) switches the gateway to Z.ai mode, whose companion vars are ZAI_GATEWAY_URL (defaulting to https://api.z.ai/api/paas/v4), ZAI_DEFAULT_MODEL and ZAI_FALLBACK_MODEL. Without a Z.ai credential the legacy Mistral mode keeps reading MISTRAL_GATEWAY_URL / MISTRAL_API_KEY / NOVA_MISTRAL_GATEWAY_TOKEN / MISTRAL_DEFAULT_MODEL / MISTRAL_FALLBACK_MODEL exactly as before, so switching providers is a single atomic env change (add ZAI_API_KEY) and rollback is its mirror (remove it). Credential and companion vars are always read from the same mode, so a half-migrated deployment can never pair a Z.ai key with the Mistral base URL or vice versa; discovery-based model degradation (unserved default -> vision pick -> text fallback) applies to both modes. Z.ai mode is tested end-to-end: default base URL, served and unserved ZAI_DEFAULT_MODEL overrides, ZAI_GATEWAY_URL mirror, and legacy-var isolation. Suite: 486 passed.
...
- The coding specialist (Kimi K3 via NVIDIA NIM) used to be a one-shot code generator: it never saw the real workspace, could not run anything, and Nova had to place and verify the returned code itself. code_task now runs the specialist as a true autonomous agent whenever the workspace sandbox is awake: it lists the workspace, reads the files it needs, writes complete files into the sandbox, and runs commands (installs, builds, tests) to verify its own work, iterating up to 12 tool rounds or an 8-minute budget before replying with a final summary. Its writes sync back into the durable Neon/S3 store immediately (via the existing end-of-run persist path), so later rounds in the same chat and the workspace UI see the real files, and the model-facing result now tells Nova to read the changed files back and verify them. The live progress panel narrates each autonomous step (listing files, writing index.html, running npm test...), feeding the code_task dropdown shipped earlier today.
- Graceful degradation everywhere: when the sandbox is unavailable the specialist falls back to the classic single-shot code reply; when the NIM endpoint rejects the tools payload (models without function calling) the first autonomous call detects NimToolsUnsupportedError and degrades to single-shot too. Transient failures still get one automatic retry, and config errors (missing NVIDIA_NIM_API_KEY) still surface verbatim.
- The autonomous specialist operates under explicit safety rules: workspace-relative paths only (path traversal rejected), no deletions or renames via shell (the durable store would resurrect them), command output truncated, and the last round withholds tools so the model must summarize instead of starting work it cannot finish. The system prompt and code_task tool description now describe both shapes (autonomous vs bare code) so the agent always verifies the right thing.

2026-09-21 - code_task dropdown: watch the coding specialist work

- The workspace chat rendered code_task as a plain one-line pill, so while the coding specialist (Kimi K3) spent minutes on a task the user saw nothing but "Code Task: ..." - even though the server already streams live progress ("the coding specialist is working on your task - 20s elapsed...") and persists the specialist's returned code as the activity detail. code_task now gets the same dropdown treatment as research_web: a clickable header with a chevron that starts open while the task runs, showing the full task description (with language) and the live progress notes; once it finishes, the panel holds the complete code the specialist returned under a "Specialist's code" label, or the failure reason in red when the specialist went down. Applies to both live streaming activities and persisted conversation rows; the completed panel starts collapsed and toggles via the chevron.

2026-09-20 - Review hardening: subtree-aware refusals and root-level file guards

- CodeRabbit's review of the duplicate-name guardrails flagged two real gaps, now closed. (1) create_project_template's stale-content check only looked at files directly inside the project folder - but templates scaffold most files into nested subfolders (src/, out/), so a stale scaffold could hide there and still collide. The check now walks the full folder subtree below the project root and refuses if any file or subfolder exists anywhere beneath it; a completely empty folder is still reused and filled. (2) Postgres unique indexes treat NULL as distinct, so root-level files (folderId NULL) never hit the workspace_files unique constraint - two same-named root files could coexist. The drizzle schema DSL in use (drizzle-orm 0.44.7) cannot express NULLS NOT DISTINCT, so the guard lives in the handlers instead (the agent is the only writer to the table): create_file refuses a case-insensitive same-name file at the same level (root or folder), rename_file refuses a rename onto an existing same-level name, and move_file refuses a move that would collide inside the target folder - all with actionable messages.

2026-09-20 - Duplicate-name failures: friendly refusals and real error text

- Today's production failure: create_project_template into a project folder that already existed with files threw a raw duplicate-key error on the first template file, the tool executor hid it behind "The tool call failed unexpectedly.", and the model's blind retries (three template variants, then create_folder) left a second same-named folder and a stray root file. Four fixes: (1) create_project_template now refuses up front when the existing project folder holds files, with a message the model can act on (different name, or delete the stale folder); an existing but empty folder is reused and filled. (2) create_folder refuses same-name siblings (case-insensitive) at the same level, the ambiguity that sent the later create_file to the wrong folder. (3) createWorkspaceFileForUser / updateWorkspaceFileForUser catch the Postgres unique violation (code 23505, unwrapped from Drizzle's cause chain) and return undefined, so callers report "already exists" instead of crashing the tool. (4) A tool call that still fails unexpectedly now reports the real error with its cause chain (capped at 500 chars, same as inference errors), so the model can react to the actual failure instead of a canned line.
