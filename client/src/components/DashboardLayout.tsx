...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="dashboard-shell flex h-svh min-h-0 min-w-0 flex-col overflow-hidden bg-background text-foreground pt-[env(safe-area-inset-top)]">
      ...
      <aside aria-label="Workspace navigation" aria-hidden={!mobileNavOpen} className={`fixed bottom-0 left-0 z-50 flex w-[248px] flex-col border-r border-border bg-muted p-3 pb-[env(safe-area-inset-bottom)] transition-transform duration-200 ease-out md:hidden top-[calc(3.5rem+env(safe-area-inset-top))] dark:border-white/5 dark:bg-card ${mobileNavOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"}`}>
