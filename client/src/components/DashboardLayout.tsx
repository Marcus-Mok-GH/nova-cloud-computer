...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!user) return <div className="flex min-h-dvh items-center justify-center bg-white px-4 py-6 dark:bg-neutral-950"><div className="w-full max-w-sm rounded-3xl border border-neutral-200 bg-white p-6 text-center shadow-[0_24px_80px_rgba(10,10,10,0.08)] sm:p-8 dark:border-white/10 dark:bg-neutral-900"><NovaMark size={34} className="mx-auto" /><h1 className="mt-5 text-2xl font-extrabold tracking-tight">Sign in to continue</h1><p className="mt-2 text-sm leading-6 text-neutral-500 dark:text-neutral-400">Your private computer and its agent are available after passwordless sign-in.</p><button onClick={() => window.location.assign("/sign-in")} className="pill-btn pill-btn-primary mt-6 w-full">Sign in</button></div></div>;
  ...
  return (
    <div className="flex min-h-dvh min-w-0 flex-col overflow-hidden bg-[#fafafa] text-neutral-950 dark:bg-neutral-950 dark:text-neutral-50">
      ...
      <nav aria-label="Workspace navigation" className="fixed inset-x-0 bottom-0 z-50 border-t border-neutral-200/90 bg-white/95 px-1.5 pb-[max(0.55rem,env(safe-area-inset-bottom))] pt-1.5 shadow-[0_-8px_24px_rgba(10,10,10,0.04)] backdrop-blur-xl sm:px-2 sm:pt-2 dark:border-white/10 dark:bg-neutral-950/95 dark:shadow-none">
