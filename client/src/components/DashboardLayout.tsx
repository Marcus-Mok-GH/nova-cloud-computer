...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="dashboard-shell flex h-svh min-h-0 min-w-0 flex-col overflow-hidden bg-background text-foreground">
      ...
      <aside aria-label="Workspace navigation" aria-hidden={!mobileNavOpen} className={`fixed inset-y-14 bottom-0 left-0 z-50 flex w-[248px] flex-col border-r border-border bg-muted/60 p-3 transition-transform duration-200 ease-out md:hidden dark:border-white/5 dark:bg-card/60 ${mobileNavOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"}`}>
