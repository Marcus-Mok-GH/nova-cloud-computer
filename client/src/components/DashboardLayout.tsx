...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!user) return <div className="flex min-h-dvh items-center justify-center bg-background px-4 py-6"><div className="w-full max-w-sm rounded-3xl border border-border bg-card p-6 text-center shadow-[0_24px_80px_rgba(0,0,0,0.12)] sm:p-8"><NovaLogo size={34} className="mx-auto" /><h1 className="mt-5 text-2xl font-extrabold tracking-tight text-foreground">Sign in to continue</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">Your private computer and its agent are available after passwordless sign-in.</p><button onClick={() => window.location.assign("/sign-in")} className="pill-btn pill-btn-primary mt-6 w-full">Sign in</button></div></div>;
    ...
    <div className="dashboard-shell flex h-svh min-h-0 min-w-0 flex-col overflow-hidden bg-background text-foreground pt-[env(safe-area-inset-top)]">
      <header className="sticky top-0 z-40 flex h-14 shrink-0 items-center justify-between border-b border-border bg-background/88 px-3 backdrop-blur-xl sm:px-5 lg:px-7">
        <div className="flex min-w-0 items-center gap-1">
          ...
          <button onClick={() => go("/app")} className="flex min-w-0 items-center gap-2 rounded-xl px-1.5 py-1 outline-none transition hover:bg-accent focus-visible:ring-2 focus-visible:ring-ring/60"><NovaLogo size={22} /><span className="truncate text-[15px] font-extrabold tracking-tight text-foreground">Nova</span></button>
        </div>
      ...
      {/* Mobile drawer */}
      <div className={`fixed inset-0 top-[calc(3.5rem+env(safe-area-inset-top))] z-40 bg-black/35 md:hidden ${mobileNavOpen ? "" : "pointer-events-none opacity-0"} transition-opacity duration-200`} onClick={() => setMobileNavOpen(false)} aria-hidden="true" />
      <aside aria-label="Workspace navigation" aria-hidden={!mobileNavOpen} className={`fixed bottom-0 left-0 z-50 flex w-[248px] flex-col border-r border-border bg-muted p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] transition-transform duration-200 ease-out md:hidden top-[calc(3.5rem+env(safe-area-inset-top))] dark:border-white/5 dark:bg-card ${mobileNavOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"}`}>
