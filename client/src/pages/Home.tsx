...
const features: Feature[] = [
  { icon: FolderOpen, eyebrow: "Workspace", title: "A real computer, not a chat window", body: "Nova runs on a persistent cloud sandbox with its own files and folders - so your work, context, and history are actually there the next time you show up." },
  { icon: Bot, eyebrow: "Agent", title: "An agent that finishes the job", body: "Nova doesn't just reply - it creates files, builds spreadsheets, browses the web, and messages you on Telegram. Real output, not just a suggestion." },
  { icon: Timer, eyebrow: "Automations", title: "Work that runs while you don't", body: "Turn any recurring task into a routine once, and Nova keeps it running on schedule - day or night - so it's done before you think to ask." },
];

const steps = [
  { title: "Hand Nova the work", body: "Drop in a file, describe what you need, or just say what's on your mind - no setup, no blank canvas." },
  { title: "Watch it get done", body: "Nova writes the file, sends the message, runs the automation - whatever the job actually calls for." },
  { title: "Move on, it keeps going", body: "The workspace and its routines stay live in the cloud, ready the next time you check in - even from Telegram." },
];
    ...
    <main className="min-h-screen overflow-hidden bg-[#f7f5f1] text-[#1f2529] dark:bg-[#121514] dark:text-[#f4f0e8]">
      <header className="sticky top-0 z-40 border-b border-[#dfdcd4]/90 bg-[#f7f5f1]/95 backdrop-blur dark:border-white/10 dark:bg-[#121514]/95">
        <div className="mx-auto flex h-[70px] max-w-6xl items-center justify-between gap-5 px-5 sm:px-8">
          <a href="#top" className="flex items-center gap-2.5" aria-label="Nova home">
            ...
            <span className="hidden border-l border-[#d6d2c9] pl-2.5 text-[11px] font-medium text-[#77766f] sm:inline dark:border-white/15 dark:text-[#a7aaa4]">your AI cloud computer</span>
          </a>
      ...
      <section id="top" className="mx-auto grid max-w-6xl gap-12 px-5 pb-20 pt-16 sm:px-8 sm:pt-24 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-16 lg:pb-28">
        <div className="max-w-xl">
          <p className="text-sm font-semibold tracking-[0.04em] text-[#b65f38] dark:text-[#e59468]">Your own AI cloud computer</p>
          <h1 className="mt-5 max-w-lg text-[clamp(2.8rem,6vw,5.25rem)] font-semibold leading-[0.98] tracking-[-0.065em] text-[#1f2529] dark:text-[#f4f0e8]">Give your AI a computer of its own.</h1>
          <p className="mt-6 max-w-lg text-[17px] leading-8 text-[#626660] dark:text-[#b5b8b0]">Nova is a persistent cloud computer with an agent on call 24/7 - creating files, sending messages, running automations, and turning your ideas into finished things while you do something else.</p>
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <button type="button" onClick={enterNova} className="inline-flex items-center gap-2 rounded-full bg-[#b65f38] px-5 py-3 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(182,95,56,0.2)] transition hover:-translate-y-0.5 hover:bg-[#9f4f2d]">{isAuthenticated ? "Open your Nova" : "Get your own Nova"}<ArrowUpRight className="size-4" /></button>
          ...
          <div className="mt-7 flex flex-wrap gap-x-5 gap-y-2 text-xs font-medium text-[#858780] dark:text-[#969b94]">
            <span className="inline-flex items-center gap-1.5"><Check className="size-3.5 text-[#6f8d75]" />An agent that works 24/7</span>
            <span className="inline-flex items-center gap-1.5"><Check className="size-3.5 text-[#6f8d75]" />A real computer, not just a chatbot</span>
          </div>
        </div>

        <div className="relative">
          <div className="absolute -right-8 -top-8 size-32 rounded-full bg-[#e3b59e]/35 blur-3xl dark:bg-[#a5674c]/20" />
          <div className="relative rounded-[1.5rem] border border-[#d8d5cc] bg-[#ece9e2] p-2.5 shadow-[0_14px_40px_rgba(47,46,39,0.10)] sm:rounded-[2rem] sm:p-3 sm:shadow-[0_24px_70px_rgba(47,46,39,0.12)] dark:border-white/10 dark:bg-[#1d2422] dark:shadow-none" aria-hidden="true">
            <div className="overflow-hidden rounded-[1.15rem] border border-[#d4d1c8] bg-[#fbfaf7] sm:rounded-[1.45rem] dark:border-white/10 dark:bg-[#151a19]">
              ...
              <div className="grid min-h-[300px] grid-cols-1 sm:min-h-[390px] sm:grid-cols-[132px_1fr]">
                <aside className="hidden border-r border-[#e7e4dc] bg-[#f3f1eb] p-3 sm:block dark:border-white/10 dark:bg-[#1b211f]">
