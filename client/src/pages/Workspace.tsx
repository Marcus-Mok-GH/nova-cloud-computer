...
export default function Workspace() {
  ...
  return <DashboardLayout><div className="grid min-h-[calc(100vh-10.5rem)] gap-4 xl:grid-cols-[250px_minmax(0,1fr)_280px]">
    ...
    <aside className="space-y-4">
      ...
      <section className="rounded-2xl border border-white/9 bg-[#10161f] p-4"><div className="flex items-center justify-between"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-white/45">VM activity</p><span className="text-[10px] text-white/30">Private</span></div><div className="mt-3 space-y-3">{agentVmRuns.data?.slice(0, 3).map(run => <div key={run.id} className="rounded-xl border border-white/7 bg-white/[.025] p-3"><div className="flex items-center justify-between gap-2"><p className="truncate text-xs font-medium text-white/78">{run.task}</p><span className={`text-[10px] ${run.status === "succeeded" ? "text-emerald-200" : run.status === "failed" ? "text-rose-200" : "text-amber-100"}`}>{run.status}</span></div>{run.resultSummary && <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-white/42">{run.resultSummary}</p>}{run.errorMessage && <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-rose-100/55">{run.errorMessage}</p>}{(run.status === "queued" || run.status === "running") && <button onClick={() => cancelVmRun.mutate({ id: run.id })} className="mt-2 text-[10px] font-medium text-rose-200/75 hover:text-rose-100">Cancel run</button>}</div>)}{!agentVmRuns.data?.length && <p className="text-xs leading-5 text-white/35">No agent VM work has run in this workspace.</p>}</div></section>
