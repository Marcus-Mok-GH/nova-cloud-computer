...
import { Bot, ChevronRight, FileText, Folder, FolderPlus, MoreHorizontal, Paperclip, Plus, Send, Sparkles } from "lucide-react";
...
export default function Workspace() {
  ...
  return <DashboardLayout><div className="grid min-h-[calc(100vh-10.5rem)] gap-4 xl:grid-cols-[250px_minmax(0,1fr)_280px]">
    <aside className="order-2 rounded-2xl border border-white/9 bg-[#10161f] p-3 xl:order-1">
      ...
      <div className="space-y-1"><div className="flex items-center gap-2 rounded-lg bg-cyan-300/10 px-2.5 py-2 text-sm text-cyan-100"><Folder className="size-4 text-cyan-300" />Home</div>{folders.map(folder => <div key={folder.id} className="flex items-center gap-2 rounded-lg px-2.5 py-2 text-sm text-white/62 hover:bg-white/6"><ChevronRight className="size-3 text-white/28" /><Folder className="size-4 text-amber-200" />{folder.name}</div>)}</div>
      <p className="mt-5 px-2 text-[10px] font-semibold uppercase tracking-[.16em] text-white/40">Recent files</p>
      <div className="mt-2 space-y-1">{files.length ? files.map(file => <button key={file.id} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-sm text-white/60 hover:bg-white/6 hover:text-white"><FileText className="size-4 text-sky-200" /><span className="truncate">{file.name}</span></button>) : <p className="px-2 py-3 text-xs leading-5 text-white/35">Add files or folders. Nova can help you keep them organized.</p>}</div>
