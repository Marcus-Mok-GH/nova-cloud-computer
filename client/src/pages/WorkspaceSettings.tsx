...
function ModelProviderCard() {
  ...
  return <section className="rise-in rounded-2xl border bg-card p-5 text-card-foreground shadow-[0_4px_14px_rgba(10,10,10,0.05)] sm:p-7"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-muted-foreground">AI provider</p><h2 className="mt-1 text-xl font-bold tracking-tight">Use your own AI provider</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Nova runs on its built-in AI gateway by default. Add an OpenAI-compatible provider with your own API key and every chat, automation, and background job runs on it instead - your key is encrypted at rest and never leaves the server.</p></div><div className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary ring-1 ring-primary/15"><KeyRound size={24} /></div></div>
    ...
    <div className="mt-6 space-y-4 rounded-2xl border bg-muted/20 p-4 sm:p-5">
      ...
      <div className="flex items-center justify-between gap-3 rounded-xl border bg-background/60 p-3"><div><p className="text-sm font-medium">Accepts image input</p><p className="text-xs text-muted-foreground">Turn on when the model can read images Nova attaches.</p></div><Switch checked={supportsImageInput} onCheckedChange={setSupportsImageInput} /></div>
      ...
      <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={testProvider} disabled={testCustom.isPending || !baseUrl.trim() || !apiKey.trim()}>{testCustom.isPending && <Loader2 className="animate-spin" size={15} />} Test connection</Button><Button onClick={submitProvider} disabled={createCustom.isPending || !formComplete}>{createCustom.isPending && <Loader2 className="animate-spin" size={15} />} Add provider</Button></div>
    </div>
  </section>;
}
