...
function TelegramBotCard() {
  ...
  const configure = trpc.telegram.configure.useMutation({ onSuccess: async data => { await refresh(); setChatId(data.chatId ?? ""); toast.success(`Telegram bot${data.botUsername ? ` @${data.botUsername}` : ""} connected.`); }, onError: error => toast.error(error.message) });
