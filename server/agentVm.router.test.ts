...
const start = vi.fn(async (ownerId: number, input: { task: string; code?: string }) => ownerId === 1
  ...
  : { configured: false as const, run: null, message: "Daytona is not connected yet." });
...
describe("Daytona protected router", () => {
  it("returns safe platform status and owner-scoped run history", async () => {
    ...
    expect(await owner.agentVm.status()).toMatchObject({ configured: true, policy: "configured_run_cap", allowance: { remainingRuns: 49 } });
