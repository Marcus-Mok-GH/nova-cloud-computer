...
import { getE2BClient, isE2BConfigured, runE2BTaskInPersistentSandbox, ensurePersistentSandbox } from "./e2b";
import { persistE2BWorkspace, persistWorkspaceToObjectStorage, restoreWorkspaceToE2B } from "./workspaceSync";

const ERROR_LIMIT = 1000;

function safeError(error: unknown) {
  const value = error instanceof Error ? error.message : "Nova could not complete that agent VM run.";
  return value.replace(/E2B_API_KEY\s*=\s*\S+/gi, "[private credential]").slice(0, ERROR_LIMIT);
}
...
export async function startAgentVmRun(ownerId: number, input: { task: string; code?: string }) {
  ...
  if (!client) {
    return {
      ...
      run: null,
      message: "E2B is not connected yet. An administrator must add the server-only E2B API key before Nova can start agent VMs.",
    };
  }
  ...
  try {
    ...
    const sandbox = await ensurePersistentSandbox(client, computer.workspace.id, ownerId, computer.workspace.persistentSandboxId);
    ...
    const importedFileCount = await persistE2BWorkspace(ownerId, completedSandbox);
    await persistWorkspaceToObjectStorage(ownerId);
