...
function sanitizePlan(
  ...
  const timezone = validTimezone(plan.scheduleTimezone)
    ? plan.scheduleTimezone
    : userTimezone;
  const normalized = {
    ...plan,
    name: plan.name.trim().slice(0, 120),
    scheduleTimezone: timezone,
    executionPrompt: plan.executionPrompt.trim().slice(0, 8000),
  };
  if (!validCron(normalized.scheduleCron))
    throw new Error(
      "Nova generated an invalid schedule. Please describe the timing more explicitly and try again."
    );
  if (normalized.needsClarification && !normalized.clarificationQuestion)
    throw new Error("Nova could not explain what needs clarification.");
  if (!normalized.needsClarification && normalized.confidence < 0.55)
    throw new Error(
      "Nova is not confident enough to create this automation safely. Please make the schedule and task more explicit."
    );
  if (normalized.needsClarification)
    normalized.confidence = Math.min(normalized.confidence, 0.54);
  return normalized;
}
...
function parsePlan(raw: string): PlannedAutomation {
  ...
  try {
    ...
    throw new Error(
      "Nova returned an invalid automation plan. Please try again."
    );
  }
}
...
export async function planAutomation(
  ...
  if (cleaned.length < 3)
    throw new Error("Tell Nova what you want the automation to do.");
  ...
  throw new Error(
    lastError instanceof Error
      ? lastError.message
      : "Nova could not create the automation plan. Please try again."
  );
}
