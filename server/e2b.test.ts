...
describe("E2B sandbox service", () => {
  ...
  function sandboxWith(run: E2BSandboxLike["commands"]["run"] = vi.fn(async () => ({ stdout: "", stderr: "", exitCode: 0 }))) {
    return {
      sandboxId: "sbx-test",
      commands: { run },
      files: { makeDir: vi.fn(async () => true), write: vi.fn(async () => undefined), read: vi.fn(async () => new Uint8Array()) , list: vi.fn(async () => []) },
  ...
  it("creates a bounded sandbox, uploads a scoped bundle, records its ID, and kills it", async () => {
    ...
    const run = vi.fn(async (command: string) => command.startsWith("python") ? { stdout: "analysis complete\n", stderr: "", exitCode: 0 } : { stdout: "", stderr: "", exitCode: 0 });
    ...
    expect(create).toHaveBeenCalledWith(expect.objectContaining({
      template: "base",
      timeoutMs: 1_200_000,
      metadata: expect.objectContaining({ "nova.owner": "12", "nova.workspace": "4", "nova.run": "7" }),
    }));
    expect(onSandboxCreated).toHaveBeenCalledWith("sbx-private");
    expect(write).toHaveBeenCalledWith(`${E2B_WORKSPACE_DIR}/input/2-notes.md`, "Private note");
  ...
  it("defines a private persistent sandbox with pause-and-resume lifecycle", () => {
    expect(persistentSandboxConfig(41, 7)).toEqual(expect.objectContaining({
      template: "base",
      metadata: expect.objectContaining({ "nova.owner": "7", "nova.workspace": "41", "nova.persistent": "true" }),
      timeoutMs: 3_600_000,
  ...
  it("connects to the stored sandbox instead of creating a duplicate", async () => {
    ...
    await expect(ensurePersistentSandbox(client, 41, 7, "sbx-durable")).resolves.toBe(sandbox);
    expect(connect).toHaveBeenCalledWith("sbx-durable", expect.objectContaining({ timeoutMs: 3_600_000 }));
  ...
  it("does not retry a user program that returned a non-zero exit code", async () => {
    ...
    const sandbox = sandboxWith(run);
    const client: E2BClientLike = { connect: vi.fn(async () => sandbox), create: vi.fn() };

    await expect(runE2BTaskInPersistentSandbox(client, {
      workspaceId: 41,
      ownerId: 7,
  ...
  it("allows network and process-launching Python but bounds code size", () => {
    expect(() => validateE2BCode("import requests\nrequests.get('https://example.com')")).not.toThrow();
    expect(() => validateE2BCode("import subprocess\nsubprocess.run(['ls'])")).not.toThrow();
  ...
  it("keeps bundle paths safe and strips terminal escapes from bounded output", () => {
    const bundle = buildE2BWorkspaceBundle([{ id: 4, name: "../../secrets.txt", content: "safe", mimeType: "text/plain", folderId: null }], []);
    expect(bundle.uploads[0]?.remotePath).toBe(`${E2B_WORKSPACE_DIR}/input/4-secrets.txt`);
