...
export type E2BCommandResult = Pick<CommandResult, "exitCode" | "stdout" | "stderr"> & { error?: string };
...
type E2BFilesLike = {
  write: (path: string, data: string | ArrayBuffer | Uint8Array) => Promise<unknown>;
  read: (path: string, options?: { format?: "bytes" | "text" }) => Promise<string | Uint8Array>;
...
function safeE2BError(error: unknown) {
  ...
  return message
    .replace(/(?:E2B_API_KEY|authorization|bearer)\s*[=:]\s*\S+/gi, "[private credential]")
    .slice(0, 500);
}
...
export function getE2BClient(): E2BClientLike | undefined {
  ...
  return {
    create: options => Sandbox.create({ ...(options ?? {}), apiKey }) as unknown as Promise<E2BSandboxLike>,
    connect: (sandboxId, options) => Sandbox.connect(sandboxId, { ...(options ?? {}), apiKey }) as unknown as Promise<E2BSandboxLike>,
  };
}
...
export function buildE2BWorkspaceBundle(files: E2BWorkspaceFile[], folders: E2BWorkspaceFolder[]) {
...
async function runCommand(sandbox: E2BSandboxLike, command: string, options: Record<string, unknown>): Promise<E2BCommandResult> {
  try {
    ...
    if (typeof result.exitCode === "number") {
      return {
        ...
        error: typeof result.error === "string" ? result.error : safeE2BError(error),
      };
    }
    throw error;
  }
...
async function provisionOpencodeOnSandbox(sandbox: E2BSandboxLike) {
  ...
  try {
    await runCommand(sandbox, script, { cwd: E2B_WORKSPACE_DIR, timeoutMs: 180_000 });
  } catch (error) {
    console.warn(`[E2B] opencode provisioning skipped for sandbox ${sandbox.sandboxId}: ${safeE2BError(error)}`);
  }
}

async function createOrGetPersistentSandbox(client: E2BClientLike, workspaceId: number, ownerId: number, knownSandboxId?: string | null): Promise<E2BSandboxLike> {
  if (knownSandboxId) {
    const sandbox = await client.connect(knownSandboxId, { timeoutMs: PERSISTENT_SANDBOX_TIMEOUT_MS });
...
export async function ensurePersistentSandbox(client: E2BClientLike, workspaceId: number, ownerId: number, knownSandboxId?: string | null): Promise<E2BSandboxLike> {
  const key = `${ownerId}:${workspaceId}:${knownSandboxId ?? "new"}`;
  const existing = persistentSandboxInFlight.get(key);
  if (existing) return existing;

  const pending = createOrGetPersistentSandbox(client, workspaceId, ownerId, knownSandboxId);
...
export async function recoverPersistentSandbox(client: E2BClientLike, workspaceId: number, ownerId: number, knownSandboxId?: string | null): Promise<E2BSandboxLike> {
  try {
    return await createOrGetPersistentSandbox(client, workspaceId, ownerId, knownSandboxId);
  } catch {
    return ensurePersistentSandbox(client, workspaceId, ownerId);
  }
}

async function uploadBundleToSandbox(sandbox: E2BSandboxLike, files: E2BWorkspaceFile[], folders: E2BWorkspaceFolder[], task: string, code?: string) {
  ...
  await sandbox.files.write(`${E2B_WORKSPACE_DIR}/nova-manifest.json`, JSON.stringify({ task, files: bundle.manifest }, null, 2));
  for (const file of bundle.uploads) await sandbox.files.write(file.remotePath, file.content);
  await sandbox.files.write(`${E2B_WORKSPACE_DIR}/.nova-task.py`, buildTaskScript(code));
}
...
export async function runE2BTask(client: E2BClientLike, input: E2BTaskInput): Promise<E2BTaskResult> {
  ...
  try {
    ...
    const response = await runCommand(sandbox, "python3 .nova-task.py", { cwd: E2B_WORKSPACE_DIR, timeoutMs: RUN_TIMEOUT_MS });
    const output = sanitizeE2BOutput(commandOutput(response));
    if (response.exitCode !== 0) throw new Error(output);
    return { sandboxId: sandbox.sandboxId, output, uploadedFileCount: bundle.uploads.length };
  } finally {
...
async function runTaskInPersistentSandbox(sandbox: E2BSandboxLike, input: {
  task: string;
  code?: string;
  files: E2BWorkspaceFile[];
  folders: E2BWorkspaceFolder[];
  ...
  return { sandboxId: sandbox.sandboxId, output, uploadedFileCount: Math.min(input.files.length, MAX_WORKSPACE_FILES) };
}

export async function runE2BTaskInPersistentSandbox(client: E2BClientLike, input: {
  workspaceId: number;
  ownerId: number;
  ...
  validateE2BCode(input.code);
  const sandbox = await ensurePersistentSandbox(client, input.workspaceId, input.ownerId, input.sandboxId);
  try {
    return await runTaskInPersistentSandbox(sandbox, input);
  } catch (error) {
    if (error instanceof E2BTaskExitError) throw error;

    console.warn(`[E2B] Persistent task failed for workspace ${input.workspaceId}; retrying once with automatic recovery: ${safeE2BError(error)}`);
    const recoveredSandbox = await recoverPersistentSandbox(client, input.workspaceId, input.ownerId, sandbox.sandboxId);
...
export async function initWorkspacePersistentVm(workspaceId: number, ownerId: number, knownSandboxId?: string | null): Promise<string | undefined> {
  ...
  try {
    const sandbox = await ensurePersistentSandbox(client, workspaceId, ownerId, knownSandboxId);
    return sandbox.sandboxId;
  } catch (error) {
    if (knownSandboxId) console.warn(`[E2B] Stored sandbox lookup failed for workspace ${workspaceId}: ${safeE2BError(error)}`);
    try {
      const sandbox = await ensurePersistentSandbox(client, workspaceId, ownerId);
      return sandbox.sandboxId;
    } catch (provisionError) {
      console.error(`[E2B] Persistent sandbox provisioning failed for workspace ${workspaceId}: ${safeE2BError(provisionError)}`);
      return undefined;
    }
  }
}
...
export async function runOpencodeChatInPersistentSandbox(client: E2BClientLike, input: {
  workspaceId: number;
  ownerId: number;
  sandboxId?: string | null;
  model: string;
  ...
  const runOpencode = async (target: E2BSandboxLike) => {
    ...
    const response = await runCommand(target, script, { cwd: E2B_WORKSPACE_DIR, timeoutMs: OPENCODE_CHAT_TIMEOUT_MS });
    const output = response.stdout ?? "";
    if (response.exitCode !== 0) throw new Error(`opencode exited with code ${response.exitCode}: ${safeE2BError(output || response.stderr || response.error)}`);
    ...
    for (const line of lines) {
      ...
      if (event.type === "text" && typeof event.part?.text === "string") reply.push(event.part.text);
    }
    return reply.join("");
  };
  ...
  try {
    ...
    console.warn(`[E2B] Opencode chat failed for workspace ${input.workspaceId}; retrying once with automatic recovery: ${safeE2BError(error)}`);
    const recoveredSandbox = await recoverPersistentSandbox(client, input.workspaceId, input.ownerId, sandbox.sandboxId);
    reply = await runOpencode(recoveredSandbox);
    sandboxId = recoveredSandbox.sandboxId;
  }
