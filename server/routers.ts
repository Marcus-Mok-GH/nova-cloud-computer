...
const nvidiaCompletionInput = z.object({ prompt: z.string().trim().min(3, "Describe what you want NVIDIA to help with.").max(12000) });
...
export const appRouter = router({
  ...
  telegram: router({
    ...
    modelSettings: protectedProcedure.query(({ ctx }) => getTelegramModelSettingsForUser(ctx.user.id)),
    updateModel: protectedProcedure.input(z.object({ provider: z.enum(["nvidia-nim", "custom"]), modelId: z.string().trim().min(1).max(240), customModelId: z.number().int().positive().nullable().optional() })).mutation(({ ctx, input }) => updateTelegramModelSettingsForUser(ctx.user.id, input)),
