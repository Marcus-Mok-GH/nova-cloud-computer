...
import { getTerminalStatusForUser, readTerminalForUser, resizeTerminalForUser, startTerminalForUser, stopTerminalForUser, writeTerminalForUser } from "./terminal";
...
export const appRouter = router({
  ...
  terminal: router({
    ...
    start: protectedProcedure.input(terminalSizeInput).mutation(async ({ ctx, input }) => { try { return await startTerminalForUser(ctx.user.id, input); } catch (error) { const message = error instanceof Error && error.message ? error.message : "Nova could not open the terminal."; const code = /not connected|administrator|template/i.test(message) ? "PRECONDITION_FAILED" : /Open the terminal|too large/i.test(message) ? "BAD_REQUEST" : "INTERNAL_SERVER_ERROR"; throw new TRPCError({ code, message }); } }),
    ...
    write: protectedProcedure.input(z.object({ data: z.string().min(1).max(8192) })).mutation(async ({ ctx, input }) => { try { await writeTerminalForUser(ctx.user.id, input.data); return { written: true as const }; } catch (error) { const message = error instanceof Error && error.message ? error.message : "Nova could not send that input."; throw new TRPCError({ code: /Open the terminal|too large/i.test(message) ? "BAD_REQUEST" : "INTERNAL_SERVER_ERROR", message }); } }),
    ...
    stop: protectedProcedure.mutation(async ({ ctx }) => stopTerminalForUser(ctx.user.id)),
  }),
  agentVm: router({
