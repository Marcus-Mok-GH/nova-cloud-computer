...
import { createWorkspaceFileForUser, createChatForUser, getChatForUser, getOrCreateWorkspace } from "./db";
...
export async function runUserAutomationForScheduleTask(taskUid: string, now = new Date()) {
  ...
  const missionDefinition = { ...definition, runtime: { ...runtime, chatId } };
  try {
    await database.update(userAutomations).set({ definition: missionDefinition, updatedAt: now }).where(eq(userAutomations.id, automation.id));
    const prompt = buildAutonomousMissionPrompt({ name: automation.name, instructions: automation.instructions, executionPrompt: automation.executionPrompt, args: automation.args, definition: missionDefinition, now, previousRunAt: automation.lastRunAt, previousRunError: automation.lastError });
    const result = await executeWebAgentRun({ ownerId: automation.ownerId, chatId, content: prompt, requestStartedAtMs: now.getTime() });
    const report = String(result.message?.content ?? "").trim();
    if (!report) throw new Error("Nova completed the mission without a report.");
    ...
    const nextDefinition = { ...missionDefinition, runtime: { ...runtime, chatId, lastRunAt: now.toISOString(), lastRunStatus: result.outOfBudget ? "continuing" : "completed", lastRunSummary: report.slice(-2000), lastArtifactId: artifact.id } };
    await database.update(userAutomations).set({ lastRunAt: now, lastError: null, definition: nextDefinition, updatedAt: now }).where(eq(userAutomations.id, automation.id));
