...
export async function runUserAutomationForScheduleTask(taskUid: string, now = new Date()) {
  ...
  try {
    ...
    const content = typeof message === "string" ? message : Array.isArray(message) ? message.map(part => typeof part === "string" ? part : part?.type === "text" && typeof part.text === "string" ? part.text : "").join("\n") : "Nova completed the automation without a report.";
