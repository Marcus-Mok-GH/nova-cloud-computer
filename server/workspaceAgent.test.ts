...
const createFolder = vi.fn(async (_owner: number, input: { name: string }) => ({ id: 3, ...input }));
...
describe("Nova VM-agent workspace", () => {
  ...
  it("creates a requested folder directly without a model round-trip", async () => {
    const result = await runWorkspaceAgent(7, 3, "Create a folder named Research.");
  ...
  it("deletes a requested folder directly", async () => {
    ...
    expect(result.actions).toEqual([{ kind: "folder", operation: "deleted", name: "Archive" }]);
    expect(runOpencodeChat).not.toHaveBeenCalled();
  });
  ...
  it("streams a conversational reply via the VM's opencode CLI", async () => {
    ...
    await expect(runWorkspaceAgent(7, 3, "Please reply with exactly this one word: PINEAPPLE.")).resolves.toMatchObject({
      actions: [],
      message: expect.objectContaining({ role: "assistant", content: "Hello from the VM." }),
    });

    expect(runOpencodeChat).toHaveBeenCalledWith({}, expect.objectContaining({
      workspaceId: 41,
      ownerId: 7,
      model: "big-pickle",
