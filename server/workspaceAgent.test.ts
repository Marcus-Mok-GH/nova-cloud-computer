...
describe("Nova keyless workspace agent", () => {
  ...
  it("executes folder deletion from the hosted-model tool path", async () => {
    ...
    expect(invoke).toHaveBeenCalledWith(expect.objectContaining({ apiKey: "configured-nim-key", apiUrl: "https://integrate.api.nvidia.com/v1", model: "moonshotai/kimi-k3" }));
