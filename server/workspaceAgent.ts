...
/** Below this much remaining budget the Telegram work-confirmation is skipped. */
const ACK_MIN_REMAINING_MS = 30_000;
/** The work-confirmation may hold back the agent run this long, no more. */
const ACK_MAX_WAIT_MS = 12_000;
...
/**
 * Guaranteed work-started confirmation for Telegram. The agent's ETA progress
 * notes are model-driven - when the first gateway round stalls, the user used
 * to sit in total silence until the reply (or nothing at all) arrived. This
 * confirmation is generated by a small, fast gateway call of its own, so it
 * lands within seconds of the message, every time. It is NOT hardcoded text:
 * the model writes the line and gives an honest estimate for the task.
 *
 * Bounded: skipped entirely when the run budget is nearly gone, and it never
 * stalls the main agent run for more than ACK_MAX_WAIT_MS - a slow
 * confirmation keeps generating in the background and sends when it lands.
 */
export async function sendTelegramWorkStartedAck(
  ownerId: number,
  token: string,
  chatId: string,
  userText: string,
...
Write exactly ONE short confirmation line (aim for 10-20 words) that (1) confirms you received it and are working on it, and (2) gives an honest rough time estimate for the full result: a few seconds for a quick question, up to a minute or two for file work or lookups, possibly several minutes for research, builds, deployments or other multi-step work. Do NOT answer the task itself and do not ask questions - only the confirmation line. Output just that line.`
    );
    const text = (typeof completion?.text === "string" ? completion.text : "").trim();
    if (text) await sendTelegramMessage(token, chatId, text.slice(0, 512));
  })().catch(() => {
    // A failed confirmation must never break the main run: the reply still
    // arrives, and a full gateway failure surfaces through the run itself.
  });
  await Promise.race([generate, waitFor(ACK_MAX_WAIT_MS)]);
}
